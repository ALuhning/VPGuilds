...
export async function parseEncryptionKeyNear(accountId, type, encryptionKey) {
    const keyKey = "enc_key:" + accountId + ":" + type;
    ...
    key = nacl.box.keyPair.fromSecretKey(buf);
        ...
        const buff = Buffer.from(encryptionKey, 'base64');
        if (buff.length !== nacl.box.secretKeyLength) {
            throw new Error("Given secret key has wrong length");
        }
        key = nacl.box.keyPair.fromSecretKey(buff);
